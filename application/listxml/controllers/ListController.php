...
class listxml_ListController extends  Zend_Controller_Action {
	...
	public function indexAction(){
		...
		$this->view->generateHtmlList = $ojbXmlLib->_xmlGenerateList($psXmlFileName,'col',$arrAllList, "C_XML_DATA",'PK_LIST',$this->_request->getParam('list_name',""),false,false,'edit');			
		//Tao form hien thi tieu tri loc	
		//echo $psFilterXmlString;
	...
	public function addAction(){
		...
		if($this->_request->isPost()){				
			...
			$arrParameter = array(									
									...
									'PK_LIST'						=>0,
									'C_CODE'						=>trim($ojbEfyLib->_replaceBadChar($objFilter->filter($arrInput['C_CODE']))),
									'C_NAME'						=>trim($ojbEfyLib->_replaceBadChar($objFilter->filter($arrInput['C_NAME']))),
			...
			if ($objFilter->filter($arrInput['C_CODE']) != ""){				
				...
				//kiem tra neu la tham so he thong thi dat lich backup
				$sPath = $ojbXmlLib->_xmlGetXmlTagValue($psXmlStringInDb,'data_list','path_backup');	
				
				if($sPath!=''){
					$connectSQL = new Zend_Config_Ini('./config/config.ini','dbmssql');
					$arrConfig = $connectSQL->db->config->toArray();					
					$sDatabase = $arrConfig['dbname'];
					$sStartDate = $ojbXmlLib->_xmlGetXmlTagValue($psXmlStringInDb,'data_list','start_date');
					//echo '$sStartDate:'.$sStartDate;exit;
					$sStartTime = $ojbXmlLib->_xmlGetXmlTagValue($psXmlStringInDb,'data_list','time_backup');
					if($sStartDate!=''){
						$sStartDate=substr(str_replace('/','', $ojbEfyLib->_ddmmyyyyToYYyymmdd($sStartDate)),0,8);
					}
					$iStartDate=(int)$sStartDate;
					if($sStartTime!=''){
						$sStartTime=str_replace(':','', $sStartTime).'00';
						$iStartTime = (int)$sStartTime;
					}else{
						$iStartTime = 120000;
					}
					$arrReturn = $objList->ScheduleBackupDb($sPath,$sDatabase,$iStartDate,$iStartTime,'ScheduleAutoBackupEcs_NamDan');
				}
