...
*/
function _adodb_getdate($origd=false,$fast=false,$is_gmt=true)
{
static $YRS;
global $_month_table_normal,$_month_table_leaf;
...
function adodb_date2($fmt, $d=false, $is_gmt=true)
{
	if ($d !== false) {
		if (!preg_match( 
...
*/
function adodb_date($fmt,$d=false,$is_gmt=true)
{
static $daylight;
...
*/
function adodb_mktime($hr,$min,$sec,$mon=false,$day=false,$year=false,$is_dst=false,$is_gmt=true) 
{
	if (!defined('ADODB_TEST_DATES')) {
...
function adodb_strftime($fmt, $ts=false,$is_gmt=true)
{
