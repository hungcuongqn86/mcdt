...
class G_Objects_Lib
{
    ...
    public function _uploadFile($doc_types, $file_attachs, $locationList, $sDir, $sDelimitor = ',')
    {
        $path = self::_createFolder($sDir, date('Y'), date('m'), date('d'));
        $dirTemp = G_Global::getInstance()->dirTempUpload;
        $sFileNameList = "";
        $types = explode($sDelimitor, $doc_types);
        $attachs = explode($sDelimitor, $file_attachs);
        ...
        $arrFileId = array();
        for ($i = 0; $i < $count; $i++) {
            $urlTemp = $dirTemp . $attachs[$i];
            if ($arrLocation[$i] == 1) {
                $sFullFileName = $attachs[$i];
            } else {
                $filename = @array_pop(explode('!~!', $urlTemp));
                $filename = G_Convert::getInstance()->_convertVNtoEN($filename);
                $random = mt_rand(1, 1000000);
                $fodel = date("Y") . '_' . date("m") . '_' . date("d") . "_" . date("H") . date("i") . date("u") . $random . "!~!";
                $sFullFileName = $fodel . $filename;
                $dest = $path . $sFullFileName;
            }

            //Di chuyen sang thu muc luu tru
            if (file_exists($urlTemp)) {
                array_push($arrFile, $sFullFileName);
                array_push($arrDoctype, $types[$i]);
                array_push($arrFileId, '');                
                copy($urlTemp, $dest);
                unlink($urlTemp);
            }
        }

        return array('file_names' => implode($sDelimitor, $arrFile),
                        'doc_types' => implode($sDelimitor, $arrDoctype),
                        'file_ids' => implode($sDelimitor, $arrFileId)
                    );
    }
    ...
    public function updateFile($arrValue) {
        ...
        $hdn_delete_file_server = (isset($arrValue['hdn_delete_file_server']) ? $arrValue['hdn_delete_file_server'] : '');
        $hdn_delete_file_id = (isset($arrValue['hdn_delete_file_id']) ? $arrValue['hdn_delete_file_id'] : '');

        $idea = (isset($arrValue['text_idea']) ? $arrValue['text_idea'] : '');
        $systemWorktype = (isset($arrValue['systemWorktype']) ? $arrValue['systemWorktype'] : '0');
        $sWorkType = (isset($arrValue['hdn_work_type']) ? $arrValue['hdn_work_type'] : '');
        $arrInput = array(
                    'file_attachs' => $sFileAttachList,
                    'doc_types' => $sDocTypeList,
                    'delimitor' => $sDelimitor,
                    'delete_name' => $hdn_file_deleted_list,
                    'delete_id_server' => $hdn_delete_file_server,
                    'delete_id' => $hdn_delete_file_id,
                    'file_olds' => $locationList,
                    'recordId' => $arrValue['recordId'],
                    'tableObject' => $arrValue['tableObject'],
                );

        $objFileRecord = new G_Objects_FileRecord($arrInput);
        $resp = array('errors' =>  true, 'msg' => 'Lỗi cập nhật FILE');
        if ($objFileRecord->updateFile() && $objFileRecord->updateRecord()) {
            $resp = array('errors' =>  false);
            if (isset($arrValue['sendOwner']) && $arrValue['sendOwner'] != '') {
                $objFileRecord->setRecordId($arrValue['sendOwner']);
                $objFileRecord->setTableObject('KntcSendOwner');
                if ($objFileRecord->updateRecord() == false) {
                    return array('errors' =>  true, 'msg' => 'Lỗi cập nhật FILE CHUYỂN ĐƠN VỊ');
                }
            }

            if (isset($arrValue['recordWork']) && $arrValue['recordWork'] != '') {
                $objFileRecord->setRecordId($arrValue['recordWork']);
                $objFileRecord->setTableObject('KntcRecordWork');
                $objFileRecord->setDocType('TIEN_DO');
                if ($objFileRecord->updateRecord() == false) {
                    return array('errors' =>  true, 'msg' => 'Lỗi cập nhật FILE TIẾN ĐỘ');
                }
            }
        }
        return $resp;
    ...
    public function saveRecord($sql, $arrInput) {
        $dbConnect = new G_Db();
        $arrResult = array();
        try {
        ...
        $idea = (isset($arrInput['text_idea']) ? $arrInput['text_idea'] : '');
        $systemWorktype = (isset($arrInput['systemWorktype']) ? $arrInput['systemWorktype'] : '0');
        $sWorkType = (isset($arrInput['hdn_work_type']) ? $arrInput['hdn_work_type'] : '');

        if ($arrResult && $arrResult['NEW_ID']) {
            $recordId = $arrResult['NEW_ID'];
            $recordWork = '';
            if (isset($arrInput['updateRecordWork']) && $arrInput['updateRecordWork']) {
                $userIdentity = Zend_Auth::getInstance()->getIdentity();
                $arrParameter = array(
                    'pkrecordwork' => '',
                    'FkRecord' => $recordId,
                    'dWorkDate' => date('Y/m/d H:i:s'),
                    'sWorkType' => $sWorkType,
                    'sResult' => $idea,
                    'FkStaff' => $userIdentity->PkStaff,
                    'sPositionName' => $userIdentity->sPositionCode . ' - ' . $userIdentity->sName,
                    'bSystemWorktype' => $systemWorktype,
                    'sOwnerCode' => $userIdentity->sOwnerCode,
                    'sDepartmentName' => $userIdentity->sUnitName
                );
                $rs = $dbConnect->_querySql($arrParameter, 'sp_KntcRecordWorkUpdate', false, false);
                if ($rs && $rs['NEW_ID']) {
                    $recordWork = $rs['NEW_ID'];
                } else {
                    return false;
                }
            }
        ...
        return array('NEW_ID' => $recordId, 'recordWork' => $recordWork);
    }

    public static function getItem($data, $id) {
