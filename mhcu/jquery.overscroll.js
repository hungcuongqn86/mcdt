...
(function($, o){
	...
	$.extend(o, {
		...
		constants: {
			...
			captureThreshold: 3,//SO PIXEL CHENH LECH KHI CAPTURE AND MOUSE UP
			wheelDeltaMod: -500,//TOC DO CUON CHUOT GIUA
			scrollDeltaMod: 2.4,//DO DI CHUYEN KHI DRAG
			thumbThickness: 0,
			thumbOpacity: 0.0,
			boundingBox: 1000000
		},
		...
		wheel: function(event, delta) {
			...
			if ( event.detail     ) { delta = -event.detail/100; }
			...
			if (typeof event.data.capture.index === "undefined" || --event.data.capture.index === 0 ) {
				...
				if(event.data.thumbs && !event.data.thumbs.visible) {
					...
					if(event.data.thumbs.vertical) {
						event.data.thumbs.vertical.stop(true, true).fadeTo("slow", o.constants.thumbOpacity);
					}
					if(event.data.thumbs.horizontal) {
						event.data.thumbs.horizontal.stop(true, true).fadeTo("slow", o.constants.thumbOpacity);
					}
				}
			}
