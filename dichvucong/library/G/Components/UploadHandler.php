...
class G_Components_UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null)
    {
        ...
        $this->options = array(
            ...
            'upload_dir' => realpath($globals->dirTempUpload),
            'upload_url' => $this->get_full_url() . '../../../../../attach-file/',
            'download_url' => $globals->_getCurrentHttpAndHost().'main/file/gettemp?id=',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            'create_folder_by_date' => false,
            'gen_id_file_name' => false,
    ...
    protected function get_upload_path($file_name = null, $version = null)
    {
        ...
        if ($this->options['create_folder_by_date'] && $file_name && empty($version)) {
            $path = $this->create_folder_by_date($path);
        }
    ...
    {
        if ($this->options['download_url']) {
            return $this->options['download_url'] . rawurlencode($file_name);
        } else {
            ...
            return $this->get_upload_url() . $this->get_user_path()
    ...
    {
        $file_name = $file->name;
        if (strrpos($file_name, $this->options['delemitor']))
            $file->view_name = substr($file_name, (0 - (strlen($file_name) - strrpos($file_name, $this->options['delemitor']) - strlen($this->options['delemitor']))));
        else
            $file->view_name = $file_name;
        ...
        if ($file->deleteType !== 'DELETE') {
            $file->deleteUrl .= '&_method=DELETE&old=0';
        }
                                     ...
                                     $index, $content_range)
    {
        if ($this->file_name)
            ...
            $file_name = $this->get_unique_filename(
                $file_path,
            ...
            /* $file_name = $this->_replaceBadChar($file_name);
            $file_name = $this->_convertVNtoEN($file_name);
            $prefix = date("Y").'_'.date("m").'_'.date("d")."_".date("H").date("i").date("u").mt_rand(1,1000000).$this->options['delemitor'];
                                          ...
                                          $index = null, $content_range = null)
    {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                $new_file_path = $this->gen_id_file_path_uploaded($file_path);
                rename($file_path, $new_file_path);
                $file_name_part = substr($new_file_path, (0 - (strlen($new_file_path) - strrpos($new_file_path, DIRECTORY_SEPARATOR) - strlen(DIRECTORY_SEPARATOR))));
                $file->name = $file_name_part;
                $file->url = $this->get_download_url($file->name);
    ...
    public function get_upload_url()
    {
        ...
        if ($this->options['create_folder_by_date']) {
            $upload_url .= date('Y') . '/' . date('m') . '/' . date('d') . '/';
        }
        return $upload_url;
    }
    ...
    public function gen_id_file_path_uploaded($file_path)
    {
        $dir = dirname($file_path);
        $filename = basename($file_path);
        $filename = $this->_replaceBadChar($filename);
        $filename = $this->_convertVNtoEN($filename);
       /* if ($this->options['gen_id_file_name']) {
            $prefix = date("Y") . '_' . date("m") . '_' . date("d") . "_" . date("H") . date("i") . date("u") . mt_rand(1, 1000000) . $this->options['delemitor'];
            $filename = $prefix . $filename;
        } */
        $file_path = $dir . DIRECTORY_SEPARATOR . time().$this->options['delemitor'].$filename;
